...
| 38 | "디젤→no_match" 회귀 신고 | 진단: rerank top5는 안정(디젤기차 검색됨)인데 **recommend가 3/5 no_match**(코인플립). 원인=강화된 그라운딩이 일반 질의도 과잉기권. fix: 프롬프트에 "짧은/일반 질의라도 후보에 직접 해당 DB 있으면 추천, no_match는 정말 없을 때만" 추가. answer 캐시 비우고 재측정 | '디젤' **5/5 추천**, **off-domain 기권 1.000 유지**, **응답 0.962→0.989·과잉기권 0.026→0.011** |
| 39 | embedding small→large A/B + config 엄밀화 | **A/B**: large(3072d) 풀빌드 후 측정. **검색은 large 승**(dense 0.811→0.856, rerank 0.972→0.980, recall→1.000, '디젤' 평탄화 해결) **그러나 그라운딩 응답 0.989→0.956(2회 동일=진짜)** → large가 미세구분으로 top-5에 유사후보를 더 모아 recommend 혼란. 최종 추천 품질+6.5배 비용 고려 **small 유지(revert, git으로 인덱스 즉시 복원)**. **config 엄밀화**: 코드 하드코딩(`PRICES`·`_SNIPPET`·`_COMMON_MAX`)을 rules.yaml로 이동(prices/rerank_snippet/image_common_max) | small 유지, 139 passed, 측정 기반 부정 결과 |
| 40 | 전 .md 동기화 + 보안 감사 + 출처 진단 + CI/CD | **문서**: README·CLAUDE·Nextsession·reports_upload README를 현재 상태로 동기화(옛 수치·스테일 문구 제거). **npz vs Vector DB** 엄밀 설명(912 벡터엔 exact 전수비교가 ANN보다 정확·단순, DB는 over-engineering). **보안 감사**: `.env`(키)·캐시·원본 전부 gitignore+미추적 확인, 추적 파일 키 패턴 0(단 index/는 공개 결정). **출처 진단**: evidence가 *구체 근거*가 아닌 일반 섹션(제품개요·가정)인 경우 다수 발견 → Nextsession #1로. **CI/CD**: GitHub Actions(`ci.yml`) push/PR마다 pytest+cspell, 데이터 의존 테스트는 reports_upload 없으면 자동 skip | CI 그린(test 12s·spellcheck 13s), 보안 OK, 출처 개선은 TODO |
| 41 | 출처 개선 + 원문 보기 + README/CI 다듬기 | **출처 fix**(진단: evidence 75%가 일반 '제품 개요'): `best_chunk`가 일반 템플릿 섹션 회피 — 최고 코사인 `provenance_margin`(0.08) 이내면 구체 섹션 우선 → **boilerplate 75%→6%**. 화면에 "출처: 「DB명」 보고서 · 📑 섹션" **문서 명시**. **원문 보기**: 세부정보 모달에 보고서 원문 전체 펼침(출처 검증). best_chunk는 표시 전용→검색 영향 0. **CI**: node24 액션(checkout v7·setup-node v6·setup-uv v8.2.0)으로 경고 제거, test 배지 삭제. README 가독성·예시 출처화 | 141 passed, boilerplate −69%p, CI 그린, 검색 품질 불변 |
| 42 | GitHub 마무리 (LICENSE·메타·README) | **LICENSE**: 데이터 소유자 **KEITI 저작권·all rights reserved**(허용적 OSS 아님 — `index/`에 KEITI 보고서 본문 포함). **description/topics**: gh 인증(researcherhojin)이 WRITE만이라 admin 필요 → 소유자가 GitHub 웹에서 설정(텍스트 제공). **README**: 반응적 over-cut(멀메이드·측정표까지 삭제) 후 **시니어형으로 일관 재작성**(예시·아키텍처 다이어그램·측정 사다리·구조·운영·링크), 전문 용어는 **원어**(embedding/reranker/hybrid/grounding) | LICENSE 커밋, CI 그린, GitHub 핵심 완료(description/topics만 소유자 admin 작업) |
...
- **그라운딩**(전체 파이프라인): off-domain 기권 **1.000**(15/15), 응답 정확도 **0.989**, 과잉기권 0.011.
  (과잉기권 방지 규칙 추가 후: '디젤' 같은 일반 질의도 후보에 해당 DB 있으면 추천, no_match는 정말 없을 때만 → logging #38)
