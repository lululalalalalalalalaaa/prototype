...
def recommend(client, query, ranked, usage=None):
    ...
    system_prompt = (
        ...
        "- 단, 검색어가 짧거나 일반적이어도 후보 중 그 대상에 **직접 해당하는** DB가 있으면 기권하지 말고 "
        "가장 적합한 것을 recommended로 고르세요(예: '디젤'→디젤기차/디젤승용차, '버스'→버스 수송 DB가 후보에 있으면 추천). "
        "no_match는 그 대상의 DB가 후보에 **정말 없을 때만**(지하철·트럭·수소차처럼).\n"
