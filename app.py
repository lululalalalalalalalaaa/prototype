...
def show_detail_dialog(db_name, evidence_section=None):
    """추천된 LCI DB의 세부정보 + 보고서 원문을 모달에 표시합니다.

    세부정보 5항목은 빌드 시점 precompute된 metadata, 원문은 추출 텍스트(`body`)다.
    원문을 함께 보여줘 화면의 출처(섹션·발췌)를 사용자가 직접 검증할 수 있게 한다.
    """
    st.markdown(f"### {db_name}")
    report = next((r for r in st.session_state.reports if r["db_name"] == db_name), None)
    detail = report.get("metadata") if report else None
    ...
    # 출처 검증용 — 보고서 원문(추출 텍스트) 전체를 펼쳐서 확인
    if report and report.get("body"):
        hint = f" (근거 섹션: {evidence_section})" if evidence_section else ""
        with st.expander(f"📄 보고서 원문 전체 (추출 텍스트){hint}"):
            st.caption("HWP에서 추출·정제한 본문입니다(표 셀·공정흐름도 vision 설명 포함). 화면의 출처는 여기서 옵니다.")
            st.text(report["body"])
...
def render_search_result(result):
    ...
    rec_evidence = (result.get("evidence") or {}).get(rec_name) or {}

    with st.container(border=True):
        st.markdown("**■ DB명**  (아래 버튼을 누르면 세부정보 + 보고서 원문을 볼 수 있습니다)")
        if st.button(f"🔎 {rec_name} — 세부정보·원문 보기", key="rec_detail_btn",
                     use_container_width=True):
            show_detail_dialog(rec_name, evidence_section=rec_evidence.get("section"))
        st.markdown("**■ 추천 사유**")
