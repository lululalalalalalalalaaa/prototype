...
- **출처(provenance):** `retrieve.best_chunk()`가 추천 DB에서 질의와 가장 일치하는 청크를 찾아
  `result["evidence"]`로 반환(텍스트+**섹션명**+kind+위치) → 화면에 "📑 영향평가 결과(표)"처럼 근거 위치까지 표시.
  (HWP는 페이지가 평탄화돼 없으므로 섹션명·청크 위치로 대체. LLM 사유와 별개의 *결정론적* 출처)
...
- 임계치: `similarity_floor`·`others_ratio`(others 상대임계), `rerank_pool`·`rerank_snippet`(리랭커), `image_common_max`(이미지 dedup)
