...
```
config/rules.yaml      # 설정 단일 소스(하드코딩 금지): 모델명·임계치·청킹·RRF·rerank_pool·rerank_snippet·image_common_max·prices(단가표)
...
rag/
  ...
  ingest/loaders.py    # 파일→레코드 파싱(.hwp/.txt/.md/.csv/.json), HWP 추출, clean_db_name, clean_body_text(폼 노이즈 정제)
  ingest/chunk.py      # structure_chunks(섹션 경계+표 분리, {text,section,kind}) + chunk_body(토큰 윈도우) + build_chunk_input(이름+섹션 prefix)
  ingest/images.py     # HWP BinData 이미지 추출 + 흰배경 평탄화 + vision 설명(공정흐름도). 로고/공통은 해시 dedup
  embed.py             # embed_text(질의), embed_texts(배치), get_encoder
  store.py             # save_index/load_index (npz+jsonl 아티팩트). load는 reports 구조 복원
  retrieve.py          # cosine_similarity, BM25(tokenize/bm25_scores), hybrid_rank(RRF), best_chunk(출처·섹션·위치)
  rerank.py            # rerank() — hybrid 후보를 LLM 관련도로 재정렬(폴백: 원래 순서)
  generate.py          # recommend(추천), summarize_detail(세부정보, 빌드시 metadata로 precompute)
  usage.py             # UsageTracker — 단계별 OpenAI 토큰·USD 비용 집계(단가는 rules.yaml prices)
  pipeline.py          # search() — hybrid_rank → rerank → recommend + trace(출처·토큰·타이밍·로깅)
eval/                  # golden.jsonl(105문항: 90 정답+15 no_match, 내용 질의 포함) + run_eval.py (dense|hybrid|rerank|answer)
tests/                 # 단계별 단위 테스트 + 실제 HWP 37개 회귀 테스트
```

**데이터 흐름 (build/serve 분리):**
1. **빌드(오프라인)**: `reports_upload/`에 보고서를 넣고 `scripts/build_index.py` 실행 → 파싱·정제·**구조 청킹**·임베딩.
   청크는 `structure_chunks`가 섹션(`N. 제목`)·표(`표.`)로 나눠 `{text, section, kind}`로 만들고, HWP 이미지는
   `images.image_chunks`가 vision으로 설명(kind='image', 공정흐름도)해 추가. `summarize_detail`로 세부정보 metadata도 1회 계산.
   `index/` 생성(`body_hash` 증분 — 단, 청커/정제 로직을 바꾸면 본문은 같아도 청크가 달라지니 `index/`를 비워 풀빌드).
...
- **출처(provenance):** `retrieve.best_chunk()`가 추천 DB에서 질의와 가장 일치하는 청크를 찾아
  `result["evidence"]`로 반환(텍스트+**섹션명**+kind+위치). 화면엔 "출처: 「DB명」 보고서 · 📑 영향평가 결과(표)"처럼
  *어느 문서 어디*인지 명시. **일반 템플릿 섹션**(제품 개요·가정 등 `_BOILERPLATE_SECTIONS`)은 폭넓게 매칭돼
  출처로 덜 유용 → 최고 코사인 `provenance_margin`(0.08) 이내면 구체 섹션을 우선(boilerplate 75%→6%, 측정).
  best_chunk는 **표시 전용**(검색·추천에 영향 없음). 추천 DB를 클릭하면 모달(`show_detail_dialog`)에서
  세부정보 + **보고서 원문(`body`) 전체**도 확인할 수 있다(출처 검증). HWP는 페이지가 평탄화돼 섹션명·청크 위치로 대체. LLM 사유와 별개의 *결정론적* 출처.
- **토큰/비용·로깅(관측성):** `usage.UsageTracker`가 embed/rerank/recommend의 `response.usage`를 모아
  단계별 토큰·USD 비용 산출(단가는 `config/rules.yaml`의 `prices` — 실제 단가로 조정). `pipeline.search`가 `trace`로
  담고 `rag.pipeline` 로거로 콘솔에 남긴다. recommend가 본문 전체를 받아 토큰 대부분을 차지(컨텍스트 다이어트 여지).
- **store.load_index 복원 구조:** `reports=[{db_name, body, metadata, chunks:[{text, embedding, section, kind}]}]`
  (임베딩은 npz 행→list). chunks.jsonl에 section/kind 저장. 검색은 문서 청크 중 **최고 코사인(max)**.
- **증분 빌드:** `body_hash`로 변경 없는 문서의 임베딩을 재사용(재임베딩 비용 회피).
- **구조 인식 청킹:** `structure_chunks()`가 본문을 섹션(`N. 제목`) 경계로 끊어 한 청크=한 주제로 만들고,
  표(`표.`)는 독립 청크(kind='table', 캡션이 섹션명)로 분리해 영향평가 수치 검색 정밀도를 높인다(문단 윈도우 폐기).
  섹션 헤더 없는 본문(txt 등)은 통째로 한 세그먼트가 된다(graceful).
- **이미지 처리(vision):** `images.image_chunks`가 HWP BinData 이미지를 추출→흰배경 평탄화→vision(`vision_model`,
  detail:original)으로 공정흐름도를 텍스트 설명으로 변환해 청크(kind='image')로 추가. **로고·공통 템플릿은 코퍼스
  해시 빈도로 제외**(>3문서 공통=boilerplate). ⚠️ 팔레트+투명 PNG는 평탄화 안 하면 vision이 '검은 이미지'로 받아 헛읽는다.
- **그라운딩은 LLM이 담당:** off-domain 질의(데이터에 없는 주제)는 `recommend`의 `no_match`로 기권한다
  (실측 기권 정확도 1.000). **단, '디젤' 같은 일반/짧은 질의라도 후보에 직접 해당 DB가 있으면 추천**한다(과잉기권 방지 —
  no_match는 그 대상이 *정말 없을 때만*). 코사인 `similarity_floor`(0.40)는 '다른 유사 후보' 표시 필터용일 뿐, 기권 판정이 아니다.
- **품질은 측정으로 증명:** 변경마다 `eval/run_eval.py`로 before/after를 비교한다(현재 105셋: dense 0.811→
  hybrid 0.794→**rerank 0.972**, 그라운딩 off-domain 기권 1.000·응답 0.989). 인프라성 변경은 "지표 불변"을 성공 기준으로 삼는다.
  부정 결과도 자산: **호출 병합**(품질 회귀, #33)·**embedding large**(검색↑이나 그라운딩 0.989→0.956, #39)는 측정 후 **반려**.
...
본문에 이미 들어온다(표 구조만 평탄화). **이미지(공정흐름도 등)는 `BinData` 스트림에 있어 `images.py`가 별도 추출·vision
처리**한다. 신형 `.hwpx`(압축 XML)는 미지원(이 코퍼스엔 .hwpx 없음) —
...
## 설정 (config/rules.yaml — 튜닝 값의 단일 소스, 코드 하드코딩 금지)

- `model: gpt-5.4-nano` — 추천 문구·세부정보 요약 생성용 (OpenAI chat completions, `response_format=json_object`)
- `vision_model: gpt-5.4` — 이미지(공정흐름도) 설명용. nano는 한국어 다이어그램을 못 읽어 비전 강한 모델 필요(detail:original)
- `embedding_model: text-embedding-3-small` — 유사도 검색용. (large 3072d는 A/B에서 검색↑이나 그라운딩↓로 미채택, #39)
- 임계치: `similarity_floor`·`others_ratio`(others 상대임계), `rerank_pool`·`rerank_snippet`(리랭커), `image_common_max`(이미지 dedup), `provenance_margin`(출처 구체섹션 우선)
- `prices`: 모델별 단가(USD/1M) — `usage.UsageTracker`가 토큰·비용 산출에 사용(코드 하드코딩 X)
...
- **CI** (`.github/workflows/ci.yml`): push·PR마다 `test`(uv sync + pytest)와 `spellcheck`(cspell) 실행.
  단위 테스트는 API 키 불필요(`FakeClient`), 실제 `.hwp`/이미지 의존 테스트는 `reports_upload/`(gitignore)가
  없으면 `skipif`로 자동 skip → CI에서도 green. 새 코드 추가 시 이 둘을 로컬에서 먼저 통과시킨다.
