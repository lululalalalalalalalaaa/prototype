...
평범한 말로 입력하면 임베딩 + 키워드 + LLM이 **가장 맞는 DB를 출처와 함께** 추천하고, 보고서 핵심을 요약합니다.
...
──────────────────────────────────────────────────
추천:  여객수송용 디젤기차 수송
출처:  「여객수송용 디젤기차 수송」 보고서 · 📑 공정흐름도(그림)
         "디젤 → 수송공정 → 대기배출물"        ← 보고서 그림을 vision으로 읽음
...
- **빌드/서빙 분리** — 임베딩·vision 비용은 빌드(오프라인 1회)에서만. 서빙은 `index/`(npz+jsonl)를 읽기만 해
  동시성에 안전하고, **별도 벡터DB 서버가 필요 없습니다**(파일만 배포).
- **구조를 보는 인제스션** — HWP 폼 노이즈를 정제하고 섹션·표 단위로 청킹하며, 공정흐름도 **이미지를 vision으로
  텍스트화**해 함께 색인합니다. 그래서 출처가 "📑 영향평가 결과(표)"처럼 *문서 어디서* 왔는지까지 나옵니다.
- **검증 가능한 출처** — 일반 개요가 아닌 구체 섹션을 출처로 고르고, 추천 DB를 클릭하면 **보고서 원문 전체**를
  펼쳐 화면의 근거를 직접 확인할 수 있습니다.
...
app.py                   얇은 Streamlit UI(읽기 전용) — 추천·출처·원문 보기·토큰/로깅
```
